...
{
    public partial class getrolong : Form
    {
        longcolumn Long;
       
        public getrolong(longcolumn Long1 )
        {
            InitializeComponent();
            Long = Long1;
        ...
        private void getrolong_Load(object sender, EventArgs e)
        {
            try
            {
            

                double t = double.Parse(Long.txtt.Text);
                double b = double.Parse(Long.txtb.Text);
                double H0in = double.Parse(Long.txthin.Text);
                double H0out = double.Parse(Long.txthout.Text);
                double fcu = double.Parse(Long.txtfcu.Text);
                double fy = double.Parse(Long.txtfy.Text);
                double cover = double.Parse(Long.txtc.Text);
            
                double P = double.Parse(Long.txtp.Text);
                /// use Here Alfa = 1.0 .... AS=AS'
                ///استقبل هنا k
                double Kin = double.Parse(Long.txtkin.Text);
                double Kout = double.Parse(Long.txtkout.Text);

                double Lamdain = (Kin * H0in) / (0.001 * t);
                double Lamdaout = (Kout * H0out) / (b * 0.001);
                double delta;
                ...
                if (Long.checkBox1.Checked == false)
                {
                    ///////// without External Moment

                    if (Long.typebraced.Text == "Braced")
                    {
                        if (Lamdain < 15 && Lamdaout < 15)
                        {
                            // short in both sides 
                            MessageBox.Show("Short Column in Both Sides .. Design as Short Column .. Go to (P) Design ", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Close();
                        }
                    }
                    if (Long.typebraced.Text == "UnBraced")
                    {
                        if (Lamdain < 10 && Lamdaout < 10)
                        {
                            // short in both sides 
                            MessageBox.Show("Short Column in Both Sides .. Design as Short Column .. Go to (P) Design ", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Close();
                    ...
                    if (Long.typebraced.Text == "Braced")
                    {
                        if ((15 < Lamdain) && (Lamdain <= 30) && (Lamdaout < 15)) 
                        {
                            // Buckling is In plane
                            delta = Lamdain * Lamdain * (0.001 * t) / 2000;                           
                            double Madd = P * delta;          //Without External Moment
                            double Horiz = (Madd * 1000 * 1000) / (fcu * b * t * t);
                            double Vert = (P * 1000) / (fcu * b * t);
                            this.chart1.Series["Point"].Points.AddXY(Horiz, Vert);
                            Long.pictureBox2.Visible = true;
                            Long.pictureBox1.Visible = false;
                            Long.pictureBox3.Visible = false;
                            Long.pictureBox4.Visible = false;

                            ///
                            Long.txtnum1.Enabled = true;
                            Long.txtnum2.Enabled = true;
                            Long.txtnum3.Enabled = false;
                            Long.txtnum4.Enabled = false;

                            Long.txtfai1.Enabled = true;
                            Long.txtfai2.Enabled = true;
                            Long.txtfai3.Enabled = false;
                            Long.txtfai4.Enabled = false;

                        }
                        if ((Lamdain < 15) && (15 <= Lamdaout && Lamdaout <= 30))
                        {
                            // Buckling is out of plane
                            delta = Lamdaout * Lamdaout * (0.001 * b) / 2000;
                          
                            double Madd = P * delta;      // kN.m  
                            double Horiz = (Madd * 1000 * 1000) / (fcu * b * b * t);
                            double Vert = (P * 1000) / (fcu * b * t);
                            this.chart1.Series["Point"].Points.AddXY(Horiz, Vert);
                            Long.pictureBox1.Visible = true;
                            Long.pictureBox2.Visible = false;
                            Long.pictureBox3.Visible = false;
                            Long.pictureBox4.Visible = false;

                            Long.txtnum3.Enabled = true;
                            Long.txtnum4.Enabled = true;
                            Long.txtnum1.Enabled = false;
                            Long.txtnum2.Enabled = false;

                            Long.txtfai3.Enabled = true;
                            Long.txtfai4.Enabled = true;
                            Long.txtfai1.Enabled = false;
                            Long.txtfai2.Enabled = false;
                        }
                        if (Lamdain >= 15 && Lamdain <= 30 && Lamdaout >= 15 && Lamdaout <= 30)
                        {
                            //// Long at both sides 
                            // take the Maximum
                            if (Lamdain >= Lamdaout)
                        ...
                        if (Lamdain > 30)
                        {
                            MessageBox.Show("Increase Dimensions .. (Lamada exceed the Limit) " + " ꟻin = " + Math.Round(Lamdain, 2), "Information", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            Close();
                        }
                        if (Lamdaout > 30)
                        {
                            MessageBox.Show("Increase Dimensions .. (Lamada exceed the Limit) " + " ꟻout = " + Math.Round(Lamdaout, 2), "Information", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            Close();
                        }
                    ...
                    if (Long.typebraced.Text == "UnBraced")
                    {
                        if ((10 < Lamdain) && (Lamdain <= 23) && (Lamdaout < 10)) 
                        {
                            // Buckling is In plane
                            delta = Lamdain * Lamdain * (0.001 * t) / 2000;
                        ...
                        if ((Lamdain < 10) && (10 <= Lamdaout && Lamdaout <= 23))
                        {
                            // Buckling is out of plane
                            delta = Lamdaout * Lamdaout * (0.001 * b) / 2000;
                        ...
                        if (Lamdain >= 10 && Lamdain <= 23 && Lamdaout >= 10 && Lamdaout <= 23)
                        {
                            //// Long at both sides 
                            // take the Maximum
                            if (Lamdain >= Lamdaout)
                ...
                ///////////////////////////////////// with External Moment

                if (Long.checkBox1.Checked == true)
                {

                    double M = double.Parse(Long.txtm.Text);          // EXTERNAL MOMENT
                    ...
                    if (Long.typebraced.Text == "Braced")
                    {
                        ...
                        {
                            // short in both sides 
                            MessageBox.Show("Short Column in Both Sides ... Go to (M,P) Design ", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            Close();
                        }
                    }
                    if (Long.typebraced.Text == "UnBraced")
                    ...
                    if (Long.typebraced.Text == "Braced")
                    {
                        if ((15 < Lamdain) && (Lamdain <= 30) && (Lamdaout < 15))
                        {
                            ...
                            double Madd = P * delta;              //With External Moment
                            double Mtotal = M + Madd;
                            double ecc = Mtotal / P;                  //m
                            double ratio = ecc / (0.001 * t);

                            if(ratio >= 0.5)
                            {
                                MessageBox.Show("𝑒⁄𝑡 > 0.5 .. No need for I.D  >>  Close and Click design directly ?", "Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                Long.pictureBox3.Visible = true;
                                Long.pictureBox1.Visible = false;
                                Long.pictureBox2.Visible = false;
                                Long.pictureBox4.Visible = false;
                            ...
                            {
                                double Horiz = (Mtotal * 1000 * 1000) / (fcu * b * t * t);
                                double Vert = (P * 1000) / (fcu * b * t);
                                this.chart1.Series["Point"].Points.AddXY(Horiz, Vert);
                                Long.pictureBox2.Visible = true;
                                Long.pictureBox1.Visible = false;
                        ...
                        if ((Lamdain < 15) && (15 <= Lamdaout && Lamdaout <= 30))
                        {
                            // Buckling is out of plane (Biaxial)
                            // لا بد من دخول المنحنى التفاعلي للاعمدة
                            delta = Lamdaout * Lamdaout * (0.001 * b) / 2000;
                            ...
                            double Beta = 0.9 - (0.5 * Rb);
                            if((M/aa) >= (Madd/bb))
                            {
                                // Neglect Madd and design om Mx'
                                double Mxx = M + Beta * (aa / bb) * Madd;
                                double Horiz = (Mxx * 1000 * 1000) / (fcu * b * t * t);
                                double Vert = (P * 1000) / (fcu * b * t);
                                this.chart1.Series["Point"].Points.AddXY(Horiz, Vert);
                            }
                            else if ((M/aa) < (Madd/bb))
                            {
                                // Neglect Madd and design om MY'
                                double Myy = Madd + Beta * (bb / aa) * M;
                                double Horiz = (Myy * 1000 * 1000) / (fcu * b * b * t);
                                double Vert = (P * 1000) / (fcu * b * t);
                                this.chart1.Series["Point"].Points.AddXY(Horiz, Vert);
                            }
                          
                            Long.pictureBox4.Visible = true;
                            Long.pictureBox2.Visible = false;
                            Long.pictureBox1.Visible = false;
                            Long.pictureBox3.Visible = false;
