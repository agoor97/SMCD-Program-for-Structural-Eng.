...
{
    public partial class getro : Form
    {
        PMM PMM;
        public getro(PMM PMM1)
        {
            InitializeComponent();
            PMM = PMM1;
        }

        private void getro_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                double fy = double.Parse(PMM.txtfy.Text);
                double cover = double.Parse(PMM.txtc.Text);
               ...
               if (PMM.radioButton1.Checked == true)
                {
                    //Symmertical RFT

                    txtro2send.Enabled = false;
                    double Rb = (P * 1000) / (fcu * b * t);
                    double beta = 0.9 - (Rb / 2);
                    if (beta > 0.8)
                    {
                        beta = 0.8;
                    }
                    ...
                    if ((Mx / aa) >= (My / bb))
                    {
                        // Neglect My and design on Mxx
                        double Mxx = Mx + beta * (aa / bb) * My;
                        double Horiz = (Mxx * 1000 * 1000) / (fcu * b * t * t);
                        double Vert =  (P * 1000) / (fcu * b * t);
                        this.chart1.Series["Point1"].Points.AddXY(Horiz, Vert);
                        
                    }
                    else if ((Mx / aa) < (My / bb))
                    {
                        // Neglect Mx and design on Myy
                        double Myy = My + beta * (bb / aa) * Mx;
                        double Horiz = (Myy * 1000 * 1000) / (fcu * b * t * t);
                        double Vert = (P * 1000) / (fcu * b * t);
                        this.chart1.Series["Point1"].Points.AddXY(Horiz, Vert);
                    } 
                }
                ...
                {
                    //Un Symmertical RFT
                    txtro2send.Enabled = true;
                    double Rb = (P * 1000) / (fcu * b * t);
                   
                    double alfa = double.Parse(PMM.txtalfa.Text);
                    double Mxx = alfa * Mx;
                    double Myy = alfa * My;
                    double Horiz1 = (Mxx * 1000 * 1000) / (fcu * b * t * t); // الطول الكبير
                    double Vert1 = (P * 1000) / (fcu * b * t);
                    double Horiz2 = (Myy * 1000 * 1000) / (fcu * b * b * t); // العرض الصغير
                    double Vert2 = (P * 1000) / (fcu * b * t);


                    this.chart1.Series["Point1"].Points.AddXY(Horiz1, Vert1); // supply Mxx  Red Point
                    this.chart1.Series["Point2"].Points.AddXY(Horiz2, Vert2);// supply Myy    Black 
                }
        ...
        { 
            try
            {
                ...
                if (PMM.radioButton2.Checked == true)
                {
                    if (txtro1send.Text.Trim() == "" || txtro2send.Text.Trim() == "")
                    {
                        Close();
                    }
                    ...
                    {
                        if (double.Parse(txtro1send.Text) < 1.0 || double.Parse(txtro2send.Text) < 1.0)
                        {
                            MessageBox.Show("minimum Value of ρ = 1.0", "Information", MessageBoxButtons.OK, MessageBoxIcon.Exclamation);
                            txtro1send.Focus();
                            txtro1send.SelectAll();
